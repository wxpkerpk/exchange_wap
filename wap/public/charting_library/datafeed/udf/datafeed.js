...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	.done(function(response) {
		...
		var barsCount = nodata ? 0 : data.list.length;

		var volumePresent = typeof data.list != 'undefined';
		var ohlPresent = typeof data.list != 'undefined';
		...
		for (var i = 0; i < barsCount; ++i) {
			...
			if (ohlPresent) {
				barValue.open = +data.list[i][1];
				barValue.high = +data.list[i][2];
				barValue.low = +data.list[i][3];
			} else {
				barValue.open = barValue.high = barValue.low = +barValue.close;
			}

			if (volumePresent) {
				barValue.volume = +data.list[i][5];
			}
