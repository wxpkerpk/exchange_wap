...
class datafeeds {
  ...
  getBars(symbolInfo, resolution, from, to, onHistoryCallback, onErrorCallback, firstDataRequest) {
    const onLoadedCallback = data => {
      data && data.length > 0 ? onHistoryCallback(data, { noData: false }) : onHistoryCallback([], { noData: true })
    }
    this.self.getBars(symbolInfo, resolution, from, to, onLoadedCallback, onErrorCallback, firstDataRequest)
