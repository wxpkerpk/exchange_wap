...
export default {
	...
	methods:{
        ...
        details(){
            let that = this;
            let url = that.$route.params.url.replace('http://support.huo-dun.com', '');
            let lang = localStorage.getItem('language') ? localStorage.getItem('language').split('-')[0] : 'zh';
            that.$mts.posts({
                url:'api/push/support/v2/page?url='+url+'&language='+lang,
                data:{},
                success(response){
                    if (response.status == 200) {
                        ...
                        setTimeout(()=>{
                            ...
                            that.loop('a[target=_blank]', $t, content, 0 , 
                                (c)=>{
                                    that.notice = {
                                        title: String($('.post-title a').html()).trim(),
                                        content: c
                                    }
